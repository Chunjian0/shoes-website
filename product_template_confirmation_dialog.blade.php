    ...
    window.ConfirmationDialog = {
        ...
        show: function(title, message, onConfirm, options = {}) {
            ...
            const escapeHandler = (e) => {
                if (finalOptions.escapeKeyClose && e.key === 'Escape') {
                    cancelHandler();
                }
            };
            ...
            const backdropHandler = (e) => {
                if (finalOptions.backdropClose && e.target === this.backdrop) {
                    cancelHandler();
                }
            };
            ...
            this.backdrop = document.getElementById('confirmation-backdrop');
...
window.ConfirmationDialog.warning(...);
window.ConfirmationDialog.info(...);
window.ConfirmationDialog.success(...);
...
window.ConfirmationDialog.show(
    ...
    {
        type: 'danger',                     // 对话框类型: default, danger, warning, info, success
        confirmBtnText: '删除',             // 自定义确认按钮文本
        escapeKeyClose: true,               // 是否允许按Esc键关闭对话框
        backdropClose: true,                // 是否允许点击背景关闭对话框
        focusConfirmBtn: true,              // 是否自动聚焦确认按钮
        onCancel: function() {              // 用户取消时的回调函数
            console.log('用户取消了操作');
        }
    }
);
