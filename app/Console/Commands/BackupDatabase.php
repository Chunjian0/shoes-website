...
class BackupDatabase extends Command
{
    ...
    private $backupPath;
    private $maxBackups = 7; // Keep the latest7Days of backup
    ...
    /**
     * initialization mysqldump path
     *
     * @return void
     */
    protected function initializeMysqldump()
    {
    ...
    {
        try {
            $this->info('Start backing up the database...');
            
            // initialization mysqldump path
            $this->initializeMysqldump();
            ...
            $this->line("Password length: " . (empty($password) ? "Empty!" : strlen($password)));
            $this->line("Mysqldump path: " . $this->mysqldumpPath);

            // test mysqldump Version
            $versionCommand = sprintf('"%s" --version', $this->mysqldumpPath);
            $this->info('test mysqldump Version...');
            exec($versionCommand, $versionOutput, $versionReturnVar);
            if ($versionReturnVar === 0) {
                $this->info("mysqldump Version information: " . implode("\n", $versionOutput));
            } else {
            ...
            try {
                ...
                $this->info('Database connection test succeeded!');
            ...
            // If there is a password, add password parameters
            if (!empty($password)) {
                $command = sprintf(
            ...
            $command .= ' --single-transaction --quick --no-tablespaces';

            // Output the executed command (hidden password)
            $this->info("Execute the command: " . preg_replace('/--password=([^ ]+)/', '--password=***', $command));
            ...
            $this->line("Command execution result code: " . $returnVar);
            if (!empty($output)) {
                $this->line("Command output:\n" . implode("\n", $output));
            }

            if ($returnVar !== 0) {
                // Try to execute without password parameters
                if (!empty($password)) {
                    $this->info("Try to execute without password parameters...");
                    $command = sprintf(
                        '"%s" --host=%s --user=%s -p --result-file=%s %s --single-transaction --quick --no-tablespaces',
                        str_replace('\\', '\\\\', $this->mysqldumpPath),
                        escapeshellarg($host),
                        escapeshellarg($username),
                        escapeshellarg($filepath),
            ...
            $this->info("Backup file size: " . $filesize . " bytes");
            ...
            $this->info("File content preview (forward100character):");
            $this->line(substr($fileContent, 0, 100) . "...");
            ...
            if ($zip->open($zipFilename, ZipArchive::CREATE) === TRUE) {
                if ($zip->addFile($filepath, $filename)) {
                    $this->info("File has been added to the zipped package");
                    $zip->close();
                    unlink($filepath); // Delete the originalSQLdocument
                    $this->info("Backup file compressed: " . $zipFilename);
                } else {
            ...
            $this->info("Database backup was successful:" . $zipFilename);
            return 0;
        } catch (Exception $e) {
    ...
    /**
     * Find mysqldump Path to the executable file
     *
     * @return string
     * @throws Exception
     */
    private function findMysqldump()
    {
        ...
        if ($returnVar === 0 && !empty($output)) {
            $path = trim($output[0]);
            if (file_exists($path)) {
                $this->info("from PATH Found in mysqldump: " . $path);
                return $path;
            }
        }
                    ...
                    if (stripos($line, 'Location') !== false) {
                        ...
                        if (isset($parts[3])) {
                            ...
                            if (file_exists($mysqlPath)) {
                                $this->info("Find it from the registry mysqldump: " . $mysqlPath);
                                return $mysqlPath;
                            }
                        }
                    }
        ...
        foreach ($drives as $drive) {
            foreach ($commonPaths as $path) {
                ...
                if (file_exists($fullPath)) {
                    $this->info("Find from common paths mysqldump: " . $fullPath);
                    return $fullPath;
                }
            }
        }
        ...
        if ($foundPath) {
            $this->info("Find by recursive search mysqldump: " . $foundPath);
            return $foundPath;
        }
    ...
    /**
     * Recursive search directory search mysqldump.exe
     *
    ...
    private function cleanOldBackups()
    {
        ...
        if (count($files) > $this->maxBackups) {
            ...
            foreach ($filesToDelete as $file) {
                unlink($file);
                $this->info("Delete old backups:" . basename($file));
            }
        }
    }
} 