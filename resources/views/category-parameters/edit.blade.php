...
@section('title', $category->name . ' - Edit parameters')
    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li>
            <div class="flex items-center">
                ...
                <span class="text-gray-400 ml-1 md:ml-2 text-sm font-medium">Edit parameters</span>
            </div>
        </li>
    </ol>
</nav>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div class="flex-1 min-w-0">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    edit {{ $category->name }} Parameters of
                </p>
            </div>
        </div>

        <form action="{{ route('category-parameters.update', ['category' => $category, 'parameter' => $parameter]) }}" method="POST" class="space-y-6" x-data="{ type: '{{ old('type', $parameter->type) }}' }">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $parameter->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $parameter->code) }}" required
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    @error('code')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="type" id="type" x-model="type" required
                        ...
                        <option value="text" {{ old('type', $parameter->type) === 'text' ? 'selected' : '' }}>text</option>
                        <option value="number" {{ old('type', $parameter->type) === 'number' ? 'selected' : '' }}>number</option>
                        <option value="select" {{ old('type', $parameter->type) === 'select' ? 'selected' : '' }}>Pull down to select</option>
                        <option value="radio" {{ old('type', $parameter->type) === 'radio' ? 'selected' : '' }}>Single choice</option>
                        <option value="checkbox" {{ old('type', $parameter->type) === 'checkbox' ? 'selected' : '' }}>Multiple choices</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $parameter->sort_order) }}"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    @error('sort_order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div x-show="['select', 'radio', 'checkbox'].includes(type)">
                ...
                <div class="mt-1 space-y-2">
                    <template x-for="(option, index) in JSON.parse('{{ old('options', json_encode($parameter->options ?? [''])) }}')" :key="index">
            ...
            <div>
                ...
                <input type="text" name="validation_rules" id="validation_rules" value="{{ old('validation_rules', $parameter->validation_rules) }}"
                       class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
            ...
            <div>
                ...
                <input type="text" name="default_value" id="default_value" value="{{ old('default_value', $parameter->default_value) }}"
                       class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                @error('default_value')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description', $parameter->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-4">
                <div class="flex items-center">
                    <input type="checkbox" name="is_required" id="is_required" value="1" {{ old('is_required', $parameter->is_required) ? 'checked' : '' }}
                           class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                    <label for="is_required" class="ml-2 block text-sm text-gray-700">Required parameters</label>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_searchable" id="is_searchable" value="1" {{ old('is_searchable', $parameter->is_searchable) ? 'checked' : '' }}
                           class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                    <label for="is_searchable" class="ml-2 block text-sm text-gray-700">Searchable</label>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $parameter->is_active) ? 'checked' : '' }}
                           class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
