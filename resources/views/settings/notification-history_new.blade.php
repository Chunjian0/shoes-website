                ...
                <form id="test-email-form" class="space-y-6">
                    ...
                    <div class="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
                        <div class="p-4">
                            ...
                            <div class="recipient-selector hidden" id="selector-test_email_recipient">
...
<script>
    (function() {
        // 初始化标记，确保JavaScript代码只初始化一�?        let initialized = false;

        // 主要初始化函�?        function initializeElements() {
            // 如果已经初始化，则不再重复执�?            const container = document.querySelector('.notification-history-container');
            ...
            // 初始化日期选择�?            initializeDatePicker();

            // 初始化筛选功�?            initializeFilters();

            // 初始化模态框
            initializeModal();

            // 初始化接收人选择�?            initializeRecipientSelectors();

            // 初始化测试邮件表�?            initializeTestEmailForm();

            console.log('Notification history page initialized');
        }

        // 初始化测试邮件表�?        function initializeTestEmailForm() {
            ...
            // 当通知类型改变时加载模�?            if (templateTypeSelect && messageTemplateSelect) {
                ...
                messageTemplateSelect.addEventListener('change', function() {
                    ...
                    if (selectedTemplateId) {
                        ...
                        // 重置为默认�?                        if (subjectInput && contentTextarea) {
        ...
        // 初始化日期选择�?        function initializeDatePicker() {
            const dateInput = document.getElementById('date_range');
            if (!dateInput) return;

            // 这里可以添加日期选择器的初始化代�?            // 例如使用flatpickr或其他日期选择�?        }

        // 初始化筛选功�?        function initializeFilters() {
            ...
            // 从URL获取筛选参�?            const urlParams = new URLSearchParams(window.location.search);
            const typeParam = urlParams.get('type');
            const statusParam = urlParams.get('status');
            
            // 设置初始�?            if (typeParam && typeSelect) typeSelect.value = typeParam;
            ...
            // 为收件人复选框添加事件监听�?            recipientSelectors.forEach(checkbox => {
        ...
        function applyFilters() {
            ...
            // 显示或隐�?无结�?�?            const noResultsRow = document.getElementById('no-results-row');
        ...
        // 初始化接收人选择�?        function initializeRecipientSelectors() {
            ...
            // 全选按�?            selectAllButtons.forEach(button => {
                button.addEventListener('click', function() {
                    const type = this.getAttribute('data-type');
            ...
            // 取消全选按�?            deselectAllButtons.forEach(button => {
                button.addEventListener('click', function() {
                    const type = this.getAttribute('data-type');
                    const checkboxes = document.querySelectorAll(`input[name="receivers[${type}][]"]`);
            ...
            // 搜索输入�?            searchInputs.forEach(input => {
            ...
            // 单选按钮更改事�?            recipientRadios.forEach(radio => {
            ...
            // 初始更新测试邮件接收�?            updateSelectedTestEmailRecipient();
        }
        ...
        function updateSelectedRecipients(type) {
            ...
            if (selectedContainer) {
                if (checkboxes.length === 0) {
                    selectedContainer.innerHTML = '<div class="text-sm text-gray-500 italic">未选择接收�?/div>';
                } else {
        ...
        // 更新测试邮件接收人显�?        function updateSelectedTestEmailRecipient() {
        ...
        // 移除接收�?        window.removeRecipient = function(type, email) {
        ...
        window.loadTemplates = function(selectedType) {
            ...
            // 清空选择�?            templateSelect.innerHTML = '<option value="">Loading templates...</option>';
                ...
                .then(data => {
                    ...
                    if (data.success && data.templates) {
                        ...
                        if (data.templates.length === 0) {
                            ...
                            // 如果有默认模板，自动选择�?                            if (defaultTemplateId) {
        ...
        window.loadTemplateVariables = function(templateId) {
            ...
            // 显示加载�?            variablesContainer.innerHTML = '<div class="text-sm text-gray-500">Loading variables...</div>';
        ...
        // 发送测试邮�?        function sendTestEmail() {
            ...
            // 显示加载�?            resultDiv.classList.remove('hidden', 'bg-red-50', 'bg-green-50', 'text-red-700', 'text-green-700');
            ...
            createNotificationLog(data, 'sending')
                ...
                .then(({response, data, logId}) => {
                    ...
                    // 更新通知记录状态（只有当logId存在且有效时�?                    if (logId && logId !== 'undefined' && !isNaN(parseInt(logId))) {
                        ...
                        updateNotificationLog(logId, status, errorMessage)
                            .then(() => {
                                // 使用API请求更新列表，而不是刷新页�?                                fetchNotificationLogs();
                            })
                            .catch(error => {
        ...
        function createNotificationLog(data, status = 'sending') {
            ...
            const userId = getUserIdByEmail(userEmail) || 1; // 默认使用ID�?的系统用�?            
            const logData = {
                type: data.type,
                user_id: userId, // 使用用户ID而不是邮�?                subject: data.subject,
        ...
        function getUserIdByEmail(email) {
            ...
            // 如果仍然找不到，尝试从复选框中查�?            const checkbox = document.querySelector(`input[type="checkbox"][value="${email}"]`);
            ...
            // 如果找不到用户，返回系统用户ID（通常�?�?            return 1;
        }

        // 获取通知日志列表
        function fetchNotificationLogs() {
            // 获取当前筛选条�?            const type = document.getElementById('notification_type')?.value || '';
        ...
        function updateNotificationTable(data) {
            ...
            // 添加数据�?            data.data.forEach(log => {
                const row = document.createElement('tr');
                row.className = 'hover:bg-gray-50';
                
                // 格式化日�?                const date = new Date(log.created_at);
                const formattedDate = date.toISOString().replace('T', ' ').substring(0, 19);
                
                // 设置状态样�?                let statusClass = '';
        ...
        function viewNotificationDetails(id) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // 显示错误信息（如果有�?                    if (modalError && modalErrorContainer) {
...
(function() {
    ...
    // 定义NotificationViewer�?    class NotificationViewer {
        constructor() {
        ...
        showNotification(button) {
            ...
            // 获取通知行数�?            const row = button.closest('tr');
    ...
    // 定义RecipientFilter�?    class RecipientFilter {
        constructor() {
            this.bindEvents();
        ...
        bindEvents() {
            ...
            // 为选择器内容添加阻止冒�?            document.querySelectorAll('.recipient-selector').forEach(selector => {
        ...
        populateUserLists() {
            // 获取测试邮件接收者列表中的用�?            const testEmailUsers = document.querySelectorAll('.user-list[data-type="test_email_recipient"] .user-item');
        ...
        toggleRecipientSelector(e) {
            ...
            if (selector) {
                // 如果选择器已经可见，直接切换隐藏状�?                if (!selector.classList.contains('hidden')) {
                    selector.classList.add('hidden');
                ...
                // 隐藏其他选择�?                document.querySelectorAll('.recipient-selector').forEach(el => {
                ...
                // 显示当前选择�?                selector.classList.remove('hidden');
        ...
        selectAll() {
            ...
            checkboxes.forEach(checkbox => {
                checkbox.checked = true;
                // 触发change事件以更新选中状�?                checkbox.dispatchEvent(new Event('change'));
            });
            
            // 更新已选择的接收者显�?            const recipientFilter = new RecipientFilter();
        ...
        handleOutsideClick(e) {
            ...
            openSelectors.forEach(selector => {
                ...
                // 检查点击是否在选择器内部或触发按钮�?                const isClickInside = selector.contains(e.target);
                const isClickOnButton = toggleButton && toggleButton.contains(e.target);
                
                // 如果点击在选择器外部且不是在按钮上，则关闭选择�?                if (!isClickInside && !isClickOnButton) {
                    selector.classList.add('hidden');
        ...
        // 更新已选择的接收者显示（用于复选框�?        updateSelectedRecipients(type) {
            const selectedContainer = document.getElementById(`selected-${type}`);
    ...
    function initToggleForm() {
        ...
        // 初始化测试邮件表单提�?        initTestEmailForm();
    }
    
    // 初始化测试邮件表单提�?    function initTestEmailForm() {
        const form = document.getElementById('test-email-form');
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                // 显示发送中状�?                showToast('Sending test email...', 'info');
    ...
    function showToast(message, type = 'info') {
        ...
        if (alert) {
            ...
            // 如果找不到预定义的警告框，创建一个临时的提示�?            const tempAlert = document.createElement('div');
    ...
    function refreshNotificationTable() {
        ...
        if (table) {
            // 如果使用DataTables，可以直接刷�?            if (typeof $.fn !== 'undefined' && $.fn.DataTable && $.fn.DataTable.isDataTable('#notification-table')) {
    ...
    function initTemplateSelection() {
        ...
        if (templateTypeSelect && templateSelect) {
            // 当通知类型改变时，加载相应的模�?            templateTypeSelect.addEventListener('change', function() {
                const selectedType = this.value;
                
                // 清空模板选择�?                templateSelect.innerHTML = '<option value="">Custom (No Template)</option>';
                
                // 获取选定类型的模�?                fetch(`/api/message-templates?type=${selectedType}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.success && data.templates.length > 0) {
                            ...
                            data.templates.forEach(template => {
                                ...
                                if (template.is_default) {
                                    ...
                                    // 触发change事件以加载模板内�?                                    templateSelect.dispatchEvent(new Event('change'));
                                }
                            });
                        }
                    })
            ...
            templateSelect.addEventListener('change', function() {
                ...
                if (selectedOption && selectedOption.value) {
                    // 填充主题和内�?                    subjectInput.value = selectedOption.dataset.subject || '';
                    contentTextarea.value = selectedOption.dataset.content || '';
                    ...
                    // 如果没有选择模板，清空内�?                    subjectInput.value = 'Test Email';
                    contentTextarea.value = 'This is a test email message from the system.';
                    templateVariablesContainer.classList.add('hidden');
                }
            });
    ...
    function showEmailPreview() {
        ...
        if (previewModal && previewSubject && previewContent) {
            ...
            // 处理模板变量的显�?            const templateVariables = document.querySelectorAll('#template-variables-list code');
    ...
    // 初始化函�?    function initNotificationHistory() {
        // 检查特定DOM实例是否已初始化
    ...
    // 在DOM准备好或使用Livewire兼容事件初始�?    document.addEventListener('DOMContentLoaded', initNotificationHistory);
